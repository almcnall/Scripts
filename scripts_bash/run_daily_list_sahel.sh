...
exp_num='EXP02' #dealing with output from which exp?
nx=720           #the more i can put in this bash file the less will need to be replicated between idl scripts 
ny=250
nbands=3
...
echo 'pro run_daily_'$v'' > /home/source/mcnally/scripts_idl/$out_file #first line of script
 ...
 for year in {2001..2011};do #years in exp
   #cd /gibber/lis_data/RFE2_UMDVeg/output/$exp_num/NOAH32/$year
   # cd /jower/LIS/OUTPUT/$exp_num/NOAH271/$year
   cd /jower/sandbox/mcnally/fromKnot/$exp_num/NOAH32/$year 
    for i in $( ls );do #look at the list
      year=${i:0:4} #extract the year
      date=$i       #extract the date
      echo "flip_af_gs4r_sahel,\" $year\", \" $date\",\"$exp_num\", $nx,$ny,$nbands " >> /home/source/mcnally/scripts_idl/$out_file #write to file
