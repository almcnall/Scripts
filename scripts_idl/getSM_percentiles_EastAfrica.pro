...
cnt=1
w = WINDOW(WINDOW_TITLE='OND SM percentile-drought classes',DIMENSIONS=[300,1350])
YOI =2015
m=1
for YOI = 1981,2013 do begin &$
;w = WINDOW(WINDOW_TITLE=string(YOI),DIMENSIONS=[700,700]) &$
p1 = image(CONGRID(npc[*,*,m,YOI-startyr],NX*1.8,NY*18),RGB_TABLE=65,FONT_SIZE=14, $
;p1 = image(CONGRID(npc[*,*,10,0],NX*1.8,NY*18),RGB_TABLE=65,FONT_SIZE=14, $
 ; AXIS_STYLE=0,MAP_PROJECTION='Geographic',LIMIT=[map_lry,map_ulx,map_uly,map_lrx], $
  ;IMAGE_DIMENSIONS=[map_lrx-map_ulx,map_uly-map_lry],IMAGE_LOCATION=[map_ulx,map_lry], /current, layout = [5,7,35-cnt], margin=[0.1,0.1,0.1,0.1])  &$
