pro ubtrmm_datacheck

; this script takes three hourly TRMM-v6 data (mm/hr), finds the average monthly values in the time series (2000-2009)
; and calculates a scaling factor using the FCLIMv4 monthly climatology (~60yrs) field. This factor is then applied
...
outdir   = strcompress("/jabber/LIS/Data/ubTRMMcheck/", /remove_all)
...
;    close, 1
  ...
  PON = buffer/(data_in+small) ;fclim/stm
  
    ;is this working?
    openw,1,strcompress('/home/mcnally/PON_month'+mm+'.img', /remove_all)
    writeu,1,PON
    close, 1
