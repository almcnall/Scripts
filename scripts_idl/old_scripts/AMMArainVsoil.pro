...
;**********monthly mean and stdev for ALL RAIN STATIONS*******************
;array=fltarr(5,12)
;array[*,*]=!VALUES.F_NAN
;for i=0,n_elements(month)-1 do begin
;  moindex=sm(where(smo eq month(i), count))& print, count
;  if count le 1 then continue
;  savg = mean(moindex) 
;  sstdev = stdev(moindex)
;  sStderr = stdev(moindex)/sqrt(count)
;  uCI = savg+(sStderr*1.96)
;  lCI = savg-(sStderr*1.96)
;  array[*,i]=[savg,sstdev,sStderr,uCI,lCI]
;endfor  
;p1=plot(array[0,*],title='all SM stations')
;p2=plot(array[3,*], linestyle=1,/overplot)
;p2=plot(array[4,*], linestyle=1,/overplot)


;Northwest stations****************************************
;arrayNW=fltarr(5,12)
;arrayNW[*,*]=!VALUES.F_NAN
;for j=0,n_elements(month)-1 do begin
;  moindexNW=sm(where(smo eq month(j) AND slatlons[0,*] gt 13.56 AND slatlons[1,*] lt 2.65,count))
;  if count le 1 then continue
;  savg = mean(moindexNW) ;try this when I get home 10/9/11
;  sstdev = stdev(moindexNW)
;  sStderr = stdev(moindexNW)/sqrt(count)
;  uCI = savg+(sStderr*1.96)
;  lCI = savg-(sStderr*1.96)
;  arrayNW[*,j]=[savg,sstdev,sStderr,uCI,lCI]
;endfor

;p3=plot(arrayNW[0,*], /overplot, title='all SM stations',color='blue')
;p4=plot(arrayNW[3,*], /overplot, color='blue', linestyle=2)
;p5=plot(arrayNW[4,*], /overplot, color='blue', linestyle=2)

;p5=plot(arrayNW[*,2], /overplot)

;***********************************************************
; Norther Southeast stations
;arraySE=fltarr(5,12)
;arraySE[*,*]=!VALUES.F_NAN
;for j=0,n_elements(month)-1 do begin
;  moindexSE=sm(where(smo eq month(j) AND slatlons[0,*] lt 13.56 AND slatlons[1,*] lt 2.7,count))
;  if count le 1 then continue
;  savg = mean(moindexSE) ;try this when I get home 10/9/11
;  sstdev = stdev(moindexSE)
;  sStderr = stdev(moindexSE)/sqrt(count)
;  uCI = savg+(sStderr*1.96)
;  lCI = savg-(sStderr*1.96)
;  arraySE[*,j]=[savg,sstdev,sStderr,uCI,lCI]
;endfor
;
;p6=plot(arraySE[0,*],/overplot,color='green')
;p7=plot(arraySE[3,*], /overplot,linestyle=3,color='green')
;p7b=plot(arraySE[4,*], /overplot,linestyle=3,color='green')
...
for j = 0,n_elements(month)-1 do begin
  moindexDry = sm(where(smo eq month(j) AND slatlons[1,*] lt 2.7,count)) ;this adds a lot more stations than I had before.
  if count le 1 then continue
  savg = mean(moindexDry) ;try this when I get home 10/9/11
  sstdev = stdev(moindexDry)
  sStderr = stdev(moindexDry)/sqrt(count)
  uCI = savg+(sStderr*1.96)
  lCI = savg-(sStderr*1.96)
  ;use arrayDry[0,*] later for the montly means
  arrayDry[*,j]=[savg,sstdev,sStderr,uCI,lCI]
endfor

;p3=plot(arrayDry[0,*], title='Wet vs Dry stations',color='blue')
;p4=plot(arrayDry[3,*], /overplot, color='blue', linestyle=2)
;p5=plot(arrayDry[4,*], /overplot, color='blue', linestyle=2)
...
;****************************cluster my rainfall  stations into two groups********************************
;Northwest stations 
;rarrayNW=fltarr(5,12)
;rarrayNW[*,*]=!VALUES.F_NAN
;StderrNW=fltarr(12)
;
;for j=0,n_elements(month)-1 do begin
;  rmoindexNW=rain(where(rmo eq month(j) AND rlatlons[0,*] gt 13.63 AND rlatlons[1,*] lt 2.65,count))
;  if count le 1 then continue
;  ravg = mean(rmoindexNW) ;try this when I get home 10/9/11
;  rstdev = stdev(rmoindexNW)
;  rStderr = stdev(rmoindexNW)/sqrt(count)
;  uCI = ravg+(rStderr*1.96)
;  lCI = ravg-(rStderr*1.96)
;  rarrayNW[*,j]=[ravg,rstdev,rStderr,uCI,lCI]
; ; print, [transpose(rarrayNW[*,0:2]), transpose(stderrNW[*])]
;endfor

;p12=plot(rarrayNW[0,*],color='blue',title='all rainfall stations');title='NW rainfall 2006:2008 stations'
;p13=plot(rarrayNW[3,*], /overplot,color='blue',linestyle=1)
;p13b=plot(rarrayNW[4,*], color='blue',linestyle=1,/overplot)

;Southeast stations
;rarraySE=fltarr(5,12)
;rarraySE[*,*]=!VALUES.F_NAN
;rStderrSE=fltarr(12)
;
;for j=0,n_elements(month)-1 do begin
;;  rmoindexSE=rain(where(rmo eq month(j) AND rlatlons[0,*] lt 13.55 AND rlatlons[0,*] gt 13.53 $
;;                        AND rlatlons[1,*] gt 2.69 AND rlatlons[1,*] lt 2.73,count))
;  rmoindexSE=rain(where(rmo eq month(j) AND rlatlons[0,*] lt 13.55 AND rlatlons[0,*] gt 13.53 $
;                        AND rlatlons[1,*] gt 2.69 AND rlatlons[1,*] lt 2.7,count))
;  if count le 1 then continue
;  ravg = mean(rmoindexSE) ;try this when I get home 10/9/11
;  rstdev = stdev(rmoindexSE)
;  rStderr = stdev(rmoindexSE)/sqrt(count)
;  uCI = ravg+(rStderr*1.96)
;  lCI = ravg-(rStderr*1.96)
;  rarraySE[*,j]=[ravg,rstdev,rStderr,uCI,lCI]
;endfor

;p14=plot(rarraySE[0,*],color='green',/overplot);title='NW rainfall 2006:2008 stations'
;p15=plot(rarraySE[3,*], /overplot,color='green',linestyle=2)
;p15b=plot(rarraySE[4,*], /overplot,color='green',linestyle=2)

;South Southeast stations
;rarraySSE=fltarr(5,12)
;rarraySSE[*,*]=!VALUES.F_NAN
;StderrSSE=fltarr(12)
;
;for j=0,n_elements(month)-1 do begin
;  rmoindexSSE=rain(where(rmo eq month(j) AND rlatlons[0,*] lt 13.56 AND rlatlons[0,*] gt 13.53 $
;                        AND rlatlons[1,*] gt 2.7 AND rlatlons[1,*] lt 2.73,count))
;  if count le 1 then continue
;  ravg = mean(rmoindexSSE) ;try this when I get home 10/9/11
;  rstdev = stdev(rmoindexSSE)
;  rStderr = stdev(rmoindexSSE)/sqrt(count)
;  uCI = ravg+(rStderr*1.96)
;  lCI = ravg-(rStderr*1.96)
;  rarraySSE[*,j]=[ravg,rstdev,rStderr,uCI,lCI]
;endfor

;p16 = plot(rarraySSE[0,*],color='orange',/overplot);title='NW rainfall 2006:2008 stations'
;p17 = plot(rarraySSE[3,*], /overplot,color='orange',linestyle=2)
;p17b = plot(rarraySSE[4,*], /overplot, color='orange',linestyle=2)
;
;;p10.name = 'all stations'
;p12.name = 'NW mean'
;p14.name = 'SE mean'
;p16.name = 'SSE mean'
;
;!null = legend(target=[p12,p14,p16], position=[0.2,0.3]) ; not sure how this line works...
