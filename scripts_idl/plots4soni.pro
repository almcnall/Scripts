pro plots4soni  ; indir, var ;add these when I understand how to run this from a script
...
indir = strcompress("/gibber/lis_data/"+name+"/output/"+expdir+"/NOAH32/monthcube_"+name+"/",/remove_all) 
outdir = strcompress("/gibber/lis_data/"+name+"/output/"+expdir+"/NOAH32/SMal_avg/",/remove_all)
file_mkdir, outdir

cd, indir

file_w = file_search('*{09,10,11,12,01,02,03,04}_'+name+'.img');no spaces! this had not been fixed when I started dinking...
...
for d=0, n_elements(file_w)-1 do begin;; this loop is for the months but is all the files in a year...
 ofile_SMal(d) = Outdir+strmid(file_w[d],0,6)+"_SMal.img"
end
...
FOR j = 0,n_elements(file_w)-1 do begin
  ...
  ;close,2 
    ...
    for k=0,nbands_w-1 do begin
     ...
     mean_SMal[k,count,z]=mean(ingrid_w[*,*,k],/NAN); 
