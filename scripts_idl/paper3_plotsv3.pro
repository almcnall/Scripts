...
;***********these need to be jut anomalies, not z-scores, std anoms**********
...
for m = 0,n_elements(sm01cube[0,0,*,0])-1 do begin &$
  for y = 0,n_elements(sm01cube[0,0,0,0:9])-1 do begin &$
    rpaw = rstdanom[*,*,m,y] &$
    smMW = mstdanom[*,*,m,y] &$ ;uuuhhh...why is this zero?
    sm01 = Lstdanom01[*,*,m,y] &$
    sm02 = Lstdanom02[*,*,m,y] &$
    nwet = nwstdanom[*,*,m,y] &$
...
print, transpose([ reform(mean(burkinaSM[4,6:7,*], dimension=2,/nan)),reform(mean(chadSM[4,6:7,*], dimension=2, /nan)),$
                   reform(mean(maliSM[4,6:7,*], dimension=2,/nan)),reform(mean(nigerSM[4,6:7,*], dimension=2, /nan)), $
                   reform(mean(senegalSM[4,6:7,*], dimension=2, /nan)) ] )
...
;calculate anomalies
RWanom=fltarr(nx,250,12);R-WRSI
for x = 0,nx-1 do begin &$
  for y = 0,250-1 do begin &$
    RW = rgrid[x,y,*] &$
...
NWanom=fltarr(nx,250,12);NWET-WRSI
for x = 0,nx-1 do begin &$
  for y = 0,250-1 do begin &$
    Nwet = ngrid[x,y,*] &$
    test = where(finite(nwet), count) &$
...
LWanom=fltarr(nx,250,12);why do these maps looks so silly?
for x = 0,nx-1 do begin &$
  for y = 0,250-1 do begin &$
    LW = lgrid[x,y,*] &$
...
ifile = file_search('/jabber/chg-mcnally/gvf_12mo_10KMsahel.1gd4r')
nx = 720
ny = 350
