...
climfile = file_search('/raid/chg-mcnally/FCLIM_Afr/*.img')
openr,1, climfile
readu,1, climgrid
close,1
...
p1 = image(sahel, RGB_TABLE=0,image_dimensions=[72.0,35.0], image_location=[-20,-5], $ 
 min_value=0, max_value=1300, dimensions=[nx/100,ny/100],/overplot)
c = COLORBAR(target=p1,ORIENTATION=0,/BORDER_ON, $
             POSITION=[0.3,0.04,0.7,0.07], title = 'annual rainfall (mm)',font_size=20)


p1 = MAP('Geographic',LIMIT = [-5, -20, 18, 52], /overplot)
p2 = contour(sahel,RGB_TABLE=4, longitude/10,latitude/10,mapgrid=p1,n_levels=10,/overplot)
p2.rgb_table=0
p2.n_levels=8
...
p4.xtickname = xticks
p4.font_name='times'
null = legend(target=[p1,p2,p3,p4], position=[0.2,0.3],font_size=16, font_name='times') 
...
lgr2 = LEGEND(TARGET=[p1, p2, p3], font_size=16, font_name='times')
...
p1 = plot(savg,avgnsm, '*', xrange = [0.03, 0.09], yrange = [0.03, 0.09])
p1 = plot(savg,avgnsm3, 'b*', xrange = [0.03, 0.09], yrange = [0.03, 0.09], /overplot)
p1 = plot([0.03,0.09], [0.03,0.09], /overplot)
...
p3 = plot(api, thick = 3, name = 'API','grey',linestyle = 1, /overplot)
...
p1 = where(finite(savg216), complement=nulls) & print, nulls
savg216(nulls) = mean(savg216(141:144), /nan)
p1 = regress(nsm0611,savg216, correlation=corr, sigma=sigma) & print, corr, sigma ;0.71, 0.06
p1 = regress(api,savg216, correlation=corr, sigma=sigma) & print, corr, sigma ;0.72, 0.0656
p1 = regress(api,nsm0611, correlation=corr, sigma=sigma) & print, corr, sigma ;0.78, 0.044

;make the case that there is too much heteogenity/noise to detect a internannual signal but we can still be happy with results. 
;and use these stations for a better understanding of how specific plant and soil types respond to rainfall. 
;2006, 2008, 2007 - rank soil mositure
;2006, 2008, 2007 - rank station API
;2007, 2006, 2008 - rank RFE API
;there are the years where theo's data says that there is only a 6mm difference between totals. And I think that there is differing ranks
;betweent the depths and WK and TK sites -- we just can't capture that level of detail. 

ifile = file_search('/raid/chg-mcnally/API_WK_STArain_WKTKsoil.csv')
ifile2 = file_search('/raid/chg-mcnally/API_WK_UBRFrain_WKTKsoil.csv')

staAPI = read_csv(ifile)
staAPI = reform(staAPI.field2, 36,3)
ubAPI = read_csv(ifile2)
ubAPI = reform(ubAPI.field2,36,3)
...
;*****FIGURE 5 API-NDVI-SM correlation map***********
;;***is this actually figures 7,8,9?********
;i correlate the api and nsm incorr_api_ndvi_ubrf
;why is this being all wierd??
;cfile = file_search('/raid/chg-mcnally/filterNDVI_APIcorr_200101_201232v2.img')
...
p1 = image(MAsahel, image_dimensions=[72.0,23.1], image_location=[-20,-5], dimensions=[nx/100,231/100], $
           rgb_table =0, title = 'API-MW correlation May-October 2001-2010', font_size=18, min_value=0.5,max_value=1)
c = COLORBAR(target=p1,ORIENTATION=0,/BORDER_ON, $
