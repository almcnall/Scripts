...
;these data go thru Feb 2016. Not lot of reason to plot 2016.
tic
TIME = 24;months
...
for i = TIME-1,nmos*nyrs-1 do begin &$
  ...
  buffer = mean(ROvect[*,*,i-(TIME-1):i],dimension=3,/nan) &$
  ;write out in a diff loop where I can deal with Month/Yr
  RO_24mo[*,*,i] = buffer  &$  
endfor
toc
;
...
;have I replotted these since hymap?
;Gabarone Dame
...
zmask = rebin(zamb_mask,NX, NY, NYRS*NMOS)
lmask = rebin(limp_mask,NX, NY, NYRS*NMOS)
hmask = rebin(hwan_mask,NX, NY, NYRS*NMOS)

help, RO_24MO

;;compute percent of normal per month;;
gbasin = mean(mean(zmask*ro_24mo, dimension=1, /nan),dimension=1, /nan)
kbasin = mean(mean(lmask*ro_24mo, dimension=1, /nan),dimension=1, /nan)
hbasin = mean(mean(hmask*ro_24mo, dimension=1, /nan),dimension=1, /nan)


;generate the date string with TIMEGEN and label_date
dummy = LABEL_DATE(DATE_FORMAT=['%Y-%M'])
time = TIMEGEN(START=JULDAY(1,1,1982), FINAL=JULDAY(12,31,2016), units='months')


p1 = plot(time, gbasin, layout=[1,3,1],'g', xrange=[min(time),max(time)], xtickformat='label_date', name = 'Gaborone')
p2 = plot(time, kbasin, layout=[1,3,2], 'r', /current,xrange=[min(time),max(time)], xtickformat='label_date', name='Karibe' )
p3 = plot(time, hbasin, layout=[1,3,3], 'b', /current,xrange=[min(time),max(time)], xtickformat='label_date', name='Hwane')
!null = legend(target=[p1,p2,p3], position=[0.2,0.3], orientation=1,shadow=0)

PON24g = gbasin/mean(gbasin,/nan)
PON24k = kbasin/mean(kbasin,/nan)
PON24h = hbasin/mean(hbasin,/nan)

p1 = plot(time, PON24g*100, layout=[1,3,1],'g', name = 'Gaborone, Botswanna',$
          xrange=[min(time),max(time)], xtickformat='label_date')
p2 = plot(time, PON24k*100, layout=[1,3,2], 'r', /current, name='Kariba, Zambia',$
          xrange=[min(time),max(time)], xtickformat='label_date')
p3 = plot(time, PON24h*100, layout=[1,3,3], 'b', /current, name = 'Hwane, Swaziland',$
          xrange=[min(time),max(time)], xtickformat='label_date')
!null = legend(target=[p1,p2,p3], position=[0.2,0.3], orientation=1,shadow=0)
...
PONmap = (ro_24mo/rebin(mean(ro_24mo,dimension=3,/nan),NX,NY,NMOS*NYRS))*100
anom24cube = reform(PONmap,nx,ny,nmos,nyrs)
...
w=window()
tmptr = CONTOUR(anom24cube[*,*,2,nyrs-1],FINDGEN(NX)/10. + map_ulx, FINDGEN(NY)/10. + map_lry, RGB_TABLE=ct,$
  /FILL, ASPECT_RATIO=1, C_VALUE=index,RGB_INDICES=FIX(FINDGEN(ncolors)*255./ncolors), layout=[1,1,1],$
  TITLE='Mar % of norm. water avail', MAP_PROJECTION='geographic',Xstyle=1,Ystyle=1, /CURRENT)  &$
