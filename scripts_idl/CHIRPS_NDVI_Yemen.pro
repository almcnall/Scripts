...
tmpdif = IMAGE(CONGRID(MEAN(chspi[*,*,18:32],DIMENSION=3)-MEAN(chspi[*,*,4:17],DIMENSION=3),4*NX,4*NY), $
   ...
   TITLE='CHIRPS Mar-Sep SPI Difference !C 1999-2013 Mean Minus 1985-1998 Mean',FONT_SIZE=16,/ORDER,/CURRENT, $
   MAP_PROJECTION='Geographic',LIMIT=[map_lry,map_ulx,map_uly,map_lrx], AXIS_STYLE=0, $
   IMAGE_DIMENSIONS=[map_lrx-map_ulx,map_uly-map_lry],IMAGE_LOCATION=[map_ulx,map_lry])
...
tmptr = IMAGE(CONGRID(trendmap,4*NX,4*NY),RGB_TABLE=CONGRID(make_cmap(ncolors),3,256),MAX_VALUE=7.,MIN_VALUE=-7., $
   TITLE=STRING(FORMAT='(''CHIRPS Mar-Sep Trend '',I4.4,''-2013 (mm/year)'')',startyr+nskip),FONT_SIZE=16,/ORDER,/CURRENT, $
   MAP_PROJECTION='Geographic',LIMIT=[map_lry,map_ulx,map_uly,map_lrx], AXIS_STYLE=0, $
   IMAGE_DIMENSIONS=[map_lrx-map_ulx,map_uly-map_lry],IMAGE_LOCATION=[map_ulx,map_lry])
...
for i=1,nyrs-1 do ndvi = [[[ndvi]],[[tmpafr[*,*,(i*24)+(startmo-1)*2:(i*24)+(endmo*2)-1]]]]
delvar,tmpafr
...
tmpdif = IMAGE(CONGRID((MEAN(nmax[*,*,17:29],DIMENSION=3) - MEAN(nmax[*,*,0:16],DIMENSION=3))/10000.,6*ghax,6*ghay), $
   RGB_TABLE=66,MAX_VALUE=0.1,MIN_VALUE=-0.1, $;LAYOUT=[2,1,1], $
   TITLE='GIMMS Max MJJAS NDVI !C 1999-2011 Mean Minus 1982-1998 Mean',FONT_SIZE=16,/ORDER, $
   MAP_PROJECTION='Geographic',LIMIT=[map_lry,map_ulx,map_uly,map_lrx], AXIS_STYLE=0, $
...
rn2nd = CONGRID(chirp[*,*,1:30],ghax,ghay,30)
