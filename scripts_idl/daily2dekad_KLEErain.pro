...
sdeks = fltarr(72) ;do I want other stuff in this array? how many dekads will there be in 4 years? 36*4
...
for y = 2011,2012 do begin
  for m = 1,12 do begin
    ...
    if count gt 0 then sdek1 = total(sbuffer[4+i,index[d1]], /nan) else sdek1 = emptydek
    ...
    if count gt 0 then sdek2 = total(sbuffer[4+i,index[d2]],/nan) else sdek2 = emptydek
    sdeks[cnt] = sdek2 & cnt++
    
    d3=where(sbuffer[3,index] ge 21, count) & print, count
    if count gt 0 then sdek3=total(sbuffer[4+i,index[d3]],/nan) else sdek3=emptydek
    sdeks[cnt]=sdek3 
 ...
 sdeks(zeros)=0
 ofile = strcompress(indir+'KLEE.South_Rainstation_dekad.csv', /remove_all) & print, ofile
