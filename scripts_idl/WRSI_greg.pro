...
FUNCTION GET_DEKAD_WEIGHTS,in_lgp
   ...
   iv = 0.16       ; fraction of season from initial to vegetative
   vr = 0.44       ; fraction of season from initial to reproductive
   rr = 0.76       ; fraction of season from initial to ripening
   eos = 1.0       ; fraction of season at end

   ; set kc values
   init_kc = 0.3   ; kc at initial stage
   repro_kc = 1.2  ; kc at reproductive stage
   end_kc = 0.35   ; kc at end of season
...
FUNCTION WRSI,ppt,pet, $
  ...
  if ~KEYWORD_SET(SWf) then $
    SWf = 0.45		; default SWf is 0.45 (maize) unless specified
  ...
  for i=SOS_ind,SOS_ind+LGP-1 do begin &$
    ...
    ; Calculate per dekad WRSI
      WRSI[i-SOS_ind] = ((AET/PETc[i-SOS_ind])*100.) &$
       ...
       outarr[*,i-SOS_ind]= [AET, PETc[i-SOS_ind], PAW, WRSI[i-SOS_ind]] &$
