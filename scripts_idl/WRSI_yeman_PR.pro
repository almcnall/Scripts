...
FUNCTION GET_DEKAD_WEIGHTS,in_lgp
   ...
   ; set time periods of growing season when kc changes, is this set for millet?
...
FUNCTION SOS_1d,rain_arr
   ...
   for i=0,vals-3 do begin
     ;change this part for the pastoral SOS
     ; if rain_arr(i) ge 25 AND TOTAL(rain_arr(i+1:i+2)) ge 20 then begin
      ;this might be pretty tough if chirps does crappy with very small rainfall amts
      if rain_arr(i) ge 10 AND TOTAL(rain_arr(i+1:i+2)) ge 5 then begin
   ...
   if SOS eq 60 then begin
      ...
      if rain_arr(vals-2) ge 10 AND rain_arr(vals-1) ge 5 then SOS = vals-2
...
FUNCTION WRSI,ppt,pet, $
  ...
  KCout  = KCout       ;set this var to write out the dekadal Kc values
  ...
  if ~KEYWORD_SET(SOS_ind) then $
      SOS_ind = SOS_1d(ppt)  ; the index of the SOS based on ppt make a static SOS, is this indexed at 1 or zero?
    

  WRSI = FLTARR(LGP)
  Kc = get_dekad_weights(LGP)	; get the crop coefficients
  ;SOS_ind = SOS_1d(ppt)	; the index of the SOS based on ppt make a static SOS
 
  ; set the root depth fraction, uh, since this is a fraction the max should be 1, but the reported max depth (mm) is 0.9mm (is this a mistake??)
  RD_max = 0.9
  RDf = GET_RD_FRACTION(LGP,RD_max)

  ; set some of the array variables
  SWC = RDf * FLOAT(WHC) * SWf			; set the critical soil water level
  if SOS_ind lt 35 then $
    PETc = pet[SOS_ind:(SOS_ind)+(LGP)] * kc $ ; set the crop water requirement
  else PETc = !values.f_nan
 
  SWBa = PPT[SOS_ind-1]		; set initial antecedent SWB conditions
  ;print, SWBa ;I want to check this for Agoufou.
...
; loop through for all dekads of the growing season
  for i=(SOS_ind),(SOS_ind)+LGP-1 do begin  &$
    if SOS_ind gt 36 then continue
    ; Calculate plant available water (PAW) for time period i
    ; Calculate AET per time period n  
    PAW = PPT[i] + SWBa  &$
    if PAW ge SWC[i-SOS_ind] then AET = PETc[i-SOS_ind] $ 
    else AET = ((PAW/SWC[i-SOS_ind])*PETc[i-SOS_ind])  &$
    if AET gt PAW then AET = PAW
    ;if AET gt PAW then print, 'it can happen!'
    ; Calculate resulting SWB at i   
    tmp = ((SWBa + PPT[i]) - AET) &$
    ;if tmp gt 50 then SWBa = 50 $ 
     if tmp gt WHC then SWBa = WHC $
      else begin &$
        if tmp le 0 then SWBa = 0 else SWBa = tmp  &$ 
      endelse 
    ; Calculate per dekad WRSI
      WRSI[i-SOS_ind] = ((AET/PETc[i-SOS_ind])*100.) 
    ;outarr[*,i-SOS_ind]= [PPT[i],AET, PETc[i-SOS_ind], PET[i],PAW, WRSI[i-SOS_ind], SWC[i-SOS_ind]] 
    ; Calculate sum of AET and sum of PET for seasonal WRSI
    AETs = AETs + AET 
    PETs = PETs + PETc[i-SOS_ind]
    pawout[i-SOS_ind] = PAW
