...
xycords = '/home/mcnally/luce_sites/AnGamb_lat_lon.csv';change this for the mara data
valid = QUERY_ASCII(xycords, info) & print, valid, info
line = info.lines

buffer=strarr(line)  & countries=strarr(line) & localities=strarr(line)
lats=fltarr(line)    & lons=fltarr(line)    & shp_lats=fltarr(line) & shp_lons=fltarr(line) 


country=''  & locality='' & lat=0.  & lon=0.  & shp_lat=0.  & shp_lon=0.
...
for m = 0,line-1 do begin ;for each site
  ...
  ofile=strcompress(odir+'Temp'+countries[m]+localities[m]+'.csv', /remove_all)
  openw,2,ofile
